...
@onready var scan_for_targets_timer: Timer = $"../Timers/ScanForTargetsTimer"
var attack_target = null

@onready var skeleton: Skeleton3D = $"../hazmat_soldier/HazmatSoldier/Skeleton3D"
var spine1_bone_id: int = 0
...
func _ready():
	...
	spine1_bone_id = skeleton.find_bone("mixamorig_Spine1")
	
	mob = get_parent()
#	dev_info_tag.set_name_label(mob.name)
#	state_changed.connect(dev_info_tag.set_state_label)
...
func _state_logic(delta):
	...
	# Scan for attack targets every couple seconds
	if scan_for_targets_timer.time_left <= 0:
		scan_for_attack_targets()
		scan_for_targets_timer.start()
	...
	for n in detection_area.get_overlapping_bodies():
		if n is ChMob:
			if closest_target == null:
				...
				if mob.global_position.distance_to(n.global_position) \
				< mob.global_position.distance_to(closest_target.global_position):
					closest_target = n
