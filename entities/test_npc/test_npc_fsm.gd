...
var wander_max_time: float = 3

var pause_time: float = 1.0
var pause_time_variance: float = 0.5
	...
	match state:
		states.wander:
			if mob.global_position.distance_to(wander_target) < wander_stop_distance \
				or change_state_timer.time_left <= 0:
				return states.pause
		states.pause:
			if change_state_timer.time_left <= 0:
				if item_target != null:
...
func _enter_state(_new_state, _previous_state):
	...
	match state:
		states.wander:
			change_state_timer.wait_time = wander_max_time
		...
		states.pause:
			...
			change_state_timer.wait_time = rand_gen.randf_range(pause_time - pause_time_variance, 
					pause_time + pause_time_variance)
			change_state_timer.start()
